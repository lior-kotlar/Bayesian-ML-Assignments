...
OUTPUT_DIRECTORY = 'data/output/ex5/'
INPUT_DIRECTORY = 'data/input/ex5/'
...
def load_im_data():
    ...
    dogs = np.load(os.path.join(INPUT_DIRECTORY, 'dogs.npy'))
    dogs_test = dogs[-test_amnt:]
    dogs = dogs[:-test_amnt]

    frogs = np.load(os.path.join(INPUT_DIRECTORY, 'frogs.npy'))
...
def plot_ims(image_list, title=''):
    ...
    plt.axis('off')
    plt.title(title)
    plt.savefig(OUTPUT_DIRECTORY + title.replace(' ', '_').lower() + '.png', dpi=300)
    plt.close()
